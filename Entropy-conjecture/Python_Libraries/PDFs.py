...
def poisson_vett (X,lam) :
    X1 = np.asarray (X)
    return ((np.e ** (-lam)) * (lam **(X1))) / (factorial_1(X1))
...
def pdf_gauss_arr(X, m, s):
    
    Y_i = []
    for i in X :
        y = (1/(s*np.sqrt(2*np.pi))) * np.exp(-0.5*((i-m)/s)**2)
        Y_i.append (y)

    return Y_i
def pdf_gauss_arr_2(X, m, s):
    # Numpy fa il calcolo su tutto l'array X simultaneamente!
    return (1 / (s * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((X - m) / s)**2)
