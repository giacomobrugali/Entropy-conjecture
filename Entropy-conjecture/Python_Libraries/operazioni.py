...
def errore_varianza(arr):
    """
    Calcola l'errore standard sulla varianza campionaria.
    Assumendo che i dati seguano una distribuzione Normale (Gaussiana).
    """
    n = len(arr)
    
    # 1. Calcoliamo la varianza campionaria (s^2)
    # ddof=1 è fondamentale per usare (N-1) al denominatore (correzione di Bessel)
    varianza = np.var(arr, ddof=1)
    
    # 2. Calcoliamo l'errore
    errore = varianza * np.sqrt(2 / (n - 1))
